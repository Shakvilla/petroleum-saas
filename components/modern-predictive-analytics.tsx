...
export function ModernPredictiveAnalytics({
  tenant,
}: ModernPredictiveAnalyticsProps) {
  const predictions = [
    {
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {insights.map((insight, index) => (
          <Card
            key={index}
            className="group relative overflow-hidden bg-white border-gray-200  hover:shadow-lg transition-all duration-300 hover:scale-105"
          >
            <div
              className={`absolute inset-0 bg-gradient-to-br ${insight.bgGradient} opacity-50`}
            ></div>
      ...
      <Card className="border-gray-200  bg-gradient-to-br from-white to-gray-50">
        ...
        <CardContent className="space-y-4 sm:space-y-6">
          {predictions.map((prediction, index) => (
            <Card
              key={index}
              className="group relative overflow-hidden border-gray-200 bg-gray-50 hover:shadow-xl transition-all duration-300"
            >
              <div
                className={`absolute inset-0 bg-gradient-to-r ${prediction.bgGradient} opacity-30`}
              ></div>
                ...
                <div className="flex flex-col space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <Badge
                        className={`${getPriorityColor(prediction.priority)} text-xs`}
                      >
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-gray-200">
            ...
            <Button
              variant="outline"
              className="bg-white/50 border-gray-200 hover:bg-white h-10 sm:h-11 text-sm"
            >
